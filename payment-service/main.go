...
type Payment struct {
	...
	OrderID   uuid.UUID  `gorm:"type:uuid;not null" json:"order_id"` // <-- ini diganti
...
func createPayment(db *gorm.DB) http.HandlerFunc {
	return func(rw http.ResponseWriter, r *http.Request) {
		...
		if os.Getenv("FAIL") == "true" {
			http.Error(rw, "failed to handle the request", http.StatusInternalServerError)
			return
		}
		...
		OrderID uuid.UUID `json:"order_id"` // <-- ganti jadi order_id
	}
		...
		regionBytes, err := region()
		if err != nil || len(regionBytes) == 0 {
			regionBytes = []byte("12345")
		}
		...
		if err != nil {
			http.Error(rw, "Failed to save payment: "+err.Error(), http.StatusInternalServerError)
			return
		}
